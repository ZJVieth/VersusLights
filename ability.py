...
from abilities.basic_abilities import bolt, snipe, speed_bolt, homing_bolt, volley, swirling_bolt, curved_x, blink
...
ABILITY_TABLE = {
    ...
    'Snipe': {'channel': unlock_cursor, 'release': snipe, 'cooldown': 5},
...
class AbilityHandler:
    ...
    def check_collision(self, entities: list, delta_time):
        for a in self.active:
            ...
            for entity in entities:
                ...
                if a.collider(entity):
                    a.affect(entity, delta_time)
                    if not issubclass(type(a), Area):
                        a.damage(a.hp)
            ...
            for b in self.active:
                if b is None or not b.collision or a == b or b.dead():
                    continue
                if a.collider(b):
                    a.affect(b, delta_time)
                    b.affect(a, delta_time)
