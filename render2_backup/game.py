...
from settings import HEIGHT, WIDTH, Color, FPS, GRANITY, LIGHT_MIN, LIGHT_DECAY, get_delta_time, light_min, grid_width, grid_height
...
surf_decay = pg.Surface((WIDTH, HEIGHT))
surf_decay.fill(pg.Color(0, 0, 0))
surf_decay.set_alpha(50)

surf_areas = pg.Surface((WIDTH, HEIGHT), SRCALPHA)
surf_areas.fill((0, 0, 0, 0))
...
def empty_grid():
    ...
    for x in range(0, grid_width()):
        ...
        for y in range(0, grid_height()):
            ...
            output[x].append((LIGHT_MIN, LIGHT_MIN, LIGHT_MIN))
    return output
...
def play(screen: pg.Surface, framerate_clock, controllers):
    ...
    while gameRunning:
        ...
        # apply decay
        screen.blit(surf_decay, (0, 0))

        render_areas = ah.should_render_areas()
        if not render_areas:
            for a in ah.areas:
                if a.rerender:
                    render_areas = True
        ...
        if render_areas:
            surf_areas.fill((0, 0, 0, 0))
            for a in ah.areas:
                a.render(surf_areas, grid)
                a.rerender = False

        screen.blit(surf_areas, (0, 0))
